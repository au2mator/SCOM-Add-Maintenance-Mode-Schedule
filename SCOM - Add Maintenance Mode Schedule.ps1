...
try {
    $duration = New-TimeSpan -Start $c_StartDate -End $c_EndDate | Select -ExpandProperty TotalMinutes
    Write-au2matorLog -Type INFO -Text "Calculated Duration: $duration"

    try {
        $ClassID = (Get-SCOMClassInstance -DisplayName $c_DA | select ID).ID
        ...
        try {
            ...
            Write-au2matorLog -Type INFO -Text "Maintenance Schedule added"
        }
        catch {
